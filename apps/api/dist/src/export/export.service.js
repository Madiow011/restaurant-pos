...
let ExportService = class ExportService {
    ...
    async getOrdersCSV(startDate, endDate) {
        const where = { status: 'PAID' };
        if (startDate)
            where.paidAt = { ...where.paidAt, gte: new Date(startDate) };
        if (endDate)
            where.paidAt = { ...where.paidAt, lte: new Date(endDate + 'T23:59:59') };
    ...
    async getSummaryHTML(startDate, endDate) {
        const where = { status: 'PAID' };
        if (startDate)
            where.paidAt = { ...where.paidAt, gte: new Date(startDate) };
        if (endDate)
        ...
        for (const o of orders) {
            for (const i of o.orderItems) {
                ...
                if (!prodMap.has(key))
                    prodMap.set(key, { name: key, qty: 0, revenue: 0 });
