...
export class ReceiptService {
  ...
  async getReceiptData(orderId: number) {
    ...
    return {
      ...
      items: order.orderItems.map((i: any) => ({ name: i.product.name, category: i.product.category?.name, quantity: i.quantity, unitPrice: i.unitPrice, subtotal: i.subtotal, note: i.note })),
      summary: { subtotal: order.subtotal, vatRate: order.vatRate, vatAmount: order.vatAmount, totalAmount: order.totalAmount },
    };
  }

  async generateCustomerReceiptHTML(orderId: number): Promise<string> {
    const { shop, order, items, summary } = await this.getReceiptData(orderId);
    const fmt = (n: number) => n.toLocaleString('th-TH', { minimumFractionDigits: 2 });
    const fmtDate = (d: any) => d ? new Date(d).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' }) : '-';
...
<div class="no-print" style="margin-top:16px;text-align:center"><button onclick="window.print()" style="padding:8px 24px;background:#4f46e5;color:#fff;border:none;border-radius:8px;cursor:pointer">🖨️ พิมพ์ใบเสร็จลูกค้า</button></div></div></body></html>`;
  ...
  async generateKitchenOrderHTML(orderId: number): Promise<string> {
...
<hr class="divider">
${items.map((i: any) => `<div class="item"><div><div class="item-name">${i.name}</div>${i.note?`<div class="note">⚠️ ${i.note}</div>`:''}</div><div class="item-qty">× ${i.quantity}</div></div>`).join('')}
<hr class="divider"><div style="text-align:center;font-size:16px;margin-top:8px">รวม ${items.reduce((s: number, i: any) => s+i.quantity, 0)} รายการ</div>
