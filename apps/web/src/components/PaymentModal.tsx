...
type Tab = 'promptpay' | 'stripe' | 'cash';
...
export default function PaymentModal({ orderId, total, onSuccess, onClose }: Props) {
  ...
  const loadPromptPay = async () => {
    ...
    try {
      ...
      const url = await QRCode.toDataURL(res.data.qrPayload, { width: 220, margin: 1 });
      setQrDataUrl(url);
  ...
  const handleConfirm = async (method: string) => {
    ...
    try {
      if (method === 'stripe' && stripePayment) {
        await axios.post('http://localhost:3001/payment/stripe/confirm', {
        ...
        <div className="flex border-b border-slate-700">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key as Tab)}
              className={`flex-1 py-3 text-xs font-medium transition-colors ${tab === t.key ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-slate-400 hover:text-slate-200'}`}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          {tab === 'cash' && (
            <div className="space-y-4">
              ...
              {cashInput && cashChange >= 0 && (
                <div className="flex justify-between bg-slate-700 rounded-xl px-4 py-3">
                  <span className="text-slate-400">เงินทอน</span>
                  <span className="text-emerald-400 font-bold text-xl">฿{cashChange.toFixed(2)}</span>
                </div>
              )}
